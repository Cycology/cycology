...
static void xmp_destroy(void *private_data)
{
  ...
  // The code that follows assumes that the vaddr map is stored in a sequence
  // of sequential pages starting at the mapPage address. Eventually, writing
  // a new copy of the map would require allocating blocks from the free
  // page list. The current version is a quick cheat.
